...
public partial class Player : CharacterBody2D
{
    ...
    private Node2D GunAnchor => field ??= GetNode<Node2D>("%Gun");
    ...
    public sealed override void _PhysicsProcess(double _delta)
    {
        ...
        void ApplyJump()
        {
            if (grounded && Input.IsActionJustPressed(MyInput.Jump))
        ...
        void ApplyMovement()
        {
            var direction = Input.GetAxis(MyInput.Left, MyInput.Right);
        ...
        void UpdateGun()
        {
            ...
            void MoveGun()
            {
                ...
                GunAnchor.LookAt(mouse);
            }

            void FireGun()
            {
                if (Input.IsActionPressed(MyInput.Shoot))
                    Rpc(MethodName.Shoot, this.PeerId());
            }
        }
    }
    ...
    [Rpc(CallLocal = true)]
    private void Shoot(int shooter)
    {
        var bullet = Bullet.New();
        bullet.SetAuthId(shooter);
